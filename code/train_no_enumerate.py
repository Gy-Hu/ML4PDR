...
# should equal to the dimension of labels
...
#     constraints = z3.parse_smt2_file(((problem.filename[0].split(
#         '/')[-1]).replace('.pkl', '.smt2')).replace('adj_', ''), sorts={}, decls={})
#     ig_q = constraints[:-1]
...
if __name__ == "__main__":
    ...
    '''
        ...
        for _, prob in enumerate(train_bar):
            ...
            try:
                writer.add_scalar('confusion_matrix/precision', TP.item()
                                  * 1.0/(TP.item()*1.0 + FP.item()*1.0), iteration)
            except:
                writer.add_scalar('confusion_matrix/precision', 0, iteration)
            writer.add_scalar('model_evaluate/TPR_recall',  TP.item()
                              * 1.0/(TP.item()*1.0 + FN.item()*1.0), iteration)
            writer.add_scalar('model_evaluate/FPR',  FP.item()
                              * 1.0/(FP.item()*1.0 + TN.item()*1.0), iteration)
            ...
            writer.add_scalar('model_evalute/F1-Socre', 2*TP.item()*1.0 /
                              (2*TP.item()*1.0+FP.item()*1.0+FN.item()*1.0), iteration)
